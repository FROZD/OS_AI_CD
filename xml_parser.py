...
# constants
PDF_RESOURCE_DIRECTORY = "./resources"
XML_RESSOURCE_DIRECTORY = "./xml_data'"
MODE = 'PDF'
...
if MODE == 'PDF':
    ...
    client.process("processFulltextDocument", PDF_RESOURCE_DIRECTORY, output=XML_RESSOURCE_DIRECTORY,
                   consolidate_citations=True, tei_coordinates=True, force=True, verbose=True)

# load xml files
documents = []
for file in os.listdir(XML_RESSOURCE_DIRECTORY):
...
for doc in documents:
    ...
    with open(f"{XML_RESSOURCE_DIRECTORY}/{doc}", mode='r') as handle:
        grob_data = grobid_tei_xml.parse_document_xml(handle.read())

    with open(f"{XML_RESSOURCE_DIRECTORY}/{doc}") as handle:
